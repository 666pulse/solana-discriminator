...
fn validate_file_type(path: &str) -> Result<String, String> {
    ...
    match path.extension().and_then(|ext| ext.to_str()) {
        ...
        Some(ext) => Err(format!("不支持的文件类型：.{}", ext)),
...
fn print_instructions(json: &serde_json::Value) {
...
/// 处理 JSON 文件并打印指令信息
fn process_json_file(path: &str) -> Result<(), String> {
...
fn main() -> () {
    let args = Args::parse();
    print_hash(&args.namespace, &args.name);

    if let Some(path) = args.file {
        if let Err(e) = process_json_file(&path) {
            eprintln!("{}", e);
        }
    }
    ()
